...
class PmacLTPController(PmacController):
    ...
    def GetExtraAttributePar(self, axis, name):
        ...
        if name == "feedbackmode":
            if axis == 1:
                i103value = self.pmacEth.command_inout(
                    "GetIVariable", int("%d03" % axis))
                try:
                    if int(i103value) == 13569:
                        ...
                        self._log.error(
                            "While getting feedback mode Pmac returned"
                            " some inconsistent value, please report"
                            " it to controls division.")
                        PyTango.Except.throw_exception(
                            "Value error",
                            "Pmac returned some inconsistent value, please"
                            " report it to controls division.",
                            "PmacLTPCtrl.GetExtraAttribute()")
                except ValueError:
                    self._log.error(
                        "While gettinh feedback mode Pmac returned some"
                        " inconsistent value, please report it to controls"
                        " division.")
                    PyTango.Except.throw_exception(
                        "Value error",
                        "Pmac returned some inconsistent value, please"
            ...
            if axis == 2:
                self._log.warning(
                    "Various feedback mode feature is reserved only for"
                    " top axis.")
                PyTango.Except.throw_exception(
                    "Value error",
            ...
            if axis == 1:
                if value == 1:
                    self.pmacEth.command_inout(
                        "SetIVariable", [int("%d03" % axis), 13569])
                elif value == 2:
                    self.pmacEth.command_inout(
                        "SetIVariable", [int("%d03" % axis), 13571])
                else:
                    self._log.warning(
                        "Feedback supports only two modes: use 1 for single"
                        " feedback mode or 2 for dual feedback mode.")
                    PyTango.Except.throw_exception(
                        "Value error",
                        "Wrong value, use 1 for single feedback mode or 2"
                        " for dual feedback mode.",
                        "PmacLTPCtrl.SetExtraAttribute()")
            else:
                self._log.warning(
                    "Various feedback mode feature is reserved only"
                    " for top axis.")
                PyTango.Except.throw_exception(
                    "Value error",
                    "Axis nr 2 does not support various feedback mode.",
